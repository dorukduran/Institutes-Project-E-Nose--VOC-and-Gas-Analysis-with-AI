...
experiment_files = {'Isopropanol_1-5-2_Open-Closed-Open.mat', 'Isopropanol_StepMeasurement.mat', 'Isopropanol+Ethanol_1-5-2_Open-Closed-Open.mat', 'Isopropanol+Ethanol_StepMeasurement.mat'};
experiment_descriptions = {...
    '1-5-2 Isopropanol Open-Closed-Open ', ...
    'Isopropanol Step Measurement', ...
    '1-5-2 Isopropanol+Ethanol Open-Closed-Open', ...
    'Isopropanol+Ethanol Step Measurement', ...
};
sensor_names = {'adc_TGS2600', 'adc_TGS2602', 'adc_TGS2610', 'adc_TGS2611', 'adc_TGS2620', 'humidity', 'temperature', 'NOX', 'VOC'};
reaction_time_limits = [7, 16, 7, 14] * 60; % Limits in minutes, converted to seconds
removal_time_limits = [8, 17, 8, 17] * 60; % Limits in minutes, converted to seconds

% Time series parameters
time_start = 1;
time_end = 250;
sampling_interval = 5; % Sampling interval in seconds
threshold = 3000; % Threshold value for slope changes
smoothing_window = 2; % Window size for data smoothing
scaling_factor = 100; % Scaling factor for slope
...
for exp_idx = 1:length(experiment_files)
    ...
    create_plots(data, sensor_names, time_start, time_end, sampling_interval, exp_idx, experiment_descriptions{exp_idx}, smoothing_window, scaling_factor);
    analyze_experiment(data, sensor_names, exp_idx, experiment_descriptions{exp_idx}, threshold, sampling_interval, smoothing_window, reaction_time_limits(exp_idx), removal_time_limits(exp_idx), scaling_factor);
end
...
function create_plots(data, sensor_names, time_start, time_end, sampling_interval, exp_idx, experiment_description, smoothing_window, scaling_factor)
    ...
    for sensor_idx = 1:num_sensors
        ...
        if isfield(experiment_data, sensor_name)
            ...
            slope = calculate_slope(smoothed_readings, sampling_interval) * scaling_factor; % Scale slope
            ...
            title(['Scaled Slope of Sensor Readings for ' sensor_name]);
            xlabel('Time (minutes)');
            ylabel('Slope (scaled)');
        else
    ...
    sgtitle(['Experiment ' num2str(exp ...
        _idx) ': ' experiment_description]);
end

% Main analysis function
function analyze_experiment(data, sensor_names, exp_idx, experiment_description, threshold, sampling_interval, smoothing_window, reaction_time_limit, removal_time_limit, scaling_factor)
    ...
    for sensor_idx = 1:num_sensors
        ...
        if isfield(experiment_data, sensor_name)
            ...
            slope = calculate_slope(smoothed_readings, sampling_interval) * scaling_factor;  % Calculate the slope of the smoothed readings and scale it
